...
#controls select, #controls button { background: #21262d; color: #c9d1d9; border: 1px solid #30363d; padding: 6px 12px; border-radius: 6px; font-size: 13px; -webkit-appearance: none; min-height: 36px; }
...
/* drag-to-zoom: dim the EXCLUDED sides dark so the selected range reads as
   highlighted (Plotly's default tints the excluded sides light gray, which
   looks inverted on a dark theme) */
...
<div id="controls">
  ...
  <button id="reset-btn" onclick="render()">⟲ Reset</button>
</div>
<div id="chart"></div>
...
function baseLayout(extraTopRows) {
  ...
  // extra headroom keeps the floating modebar clear of the legend rows
  var t = 34 + rows * 22 + (extraTopRows || 0) * 18;
  return {
...
function render() {
  ...
  if (show === 'deriv1' || show === 'deriv2') {
    ...
    Plotly.react('chart', traces, layout, {responsive: true, scrollZoom: true, modeBarButtonsToRemove: ["lasso2d", "select2d", "toImage"]});
    return;
  }
